...
Bounds GetBoundForAxis(Vector3 a, Vector3 C, float r, float nearZ) {
	...
	float k = sqrt(square(r) - square(c.y() - nearZ));
	for (int i = 0; i < 2; ++i) {
		...
		if (clipSphere && clipBound) {
			bounds[i] = Vector2(c.x() + k, nearZ);
		}
		//Set up for the lower bound
		v.y() = -v.y();  k = -k;
	}

	Bounds result = { .L = { bounds[1].x() * a }, .U = { bounds[0].x() * a }};
...
int main(int argc, char** argv)
{
	...
	window->RunMessageLoop([
		...
		AABox2D bb = getAxisAlignedBoundingBox(Vector4{main_viewport.view_matrix * Vector4{0,0,0,1}}.xyz(), 1.f, main_viewport.near_plane, main_viewport.projection_matrix);
